...
import { formatDate, formatTime, addFifteenMinutes } from "../utils/dateAndTime";
        ...
        {reservedSlot &&
          <div>
            ...
            <div className="grid grid-cols-5 gap-4">
              ...
              <span>{formatTime(reservedSlot.time)} - {formatTime(addFifteenMinutes(reservedSlot.time))}</span>
              <span className='text-red-500'>Expires: {reservedSlot.timestamp.toLocaleString()}</span>
            ...
            <ul>
              {confirmedSlots.map((slot, index) => (
                <li key={index} className="grid grid-cols-5 gap-4">
                  ...
                  <span>{formatTime(slot.time)} - {formatTime(addFifteenMinutes(slot.time))}</span>
                </li>
              ))}
            </ul>
