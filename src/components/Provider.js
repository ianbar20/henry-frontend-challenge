    ...
    <div className="flex items-center justify-center h-screen bg-gradient-to-b from-gray-100 to-gray-500">
      <div className="bg-white shadow-xl rounded p-12 mb-4">
        ...
        <form onSubmit={handleSubmit}>
          <input className="border-2" type="date" min={minDate} value={date} onChange={e => setDate(e.target.value)} required />
          <input className="border-2 ml-2" type="time" min={minTime} value={startTime} onChange={e => setStartTime(e.target.value)} required />
          <input className="border-2 ml-2" type="time" min={startTime} value={endTime} onChange={e => setEndTime(e.target.value)} required />
          <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold ml-2 py-2 px-4 rounded" type="submit">Submit Time Slot</button>
        </form>
      </div>
    </div>
  );
